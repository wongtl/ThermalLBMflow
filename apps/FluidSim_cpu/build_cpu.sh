...
BUILD_USE_SRUN="${BUILD_USE_SRUN:-0}"
...
# Slurm allocation options (used only when BUILD_USE_SRUN=1).
...
INSTALL_VENV_SCRIPT="$SRC_DIR/apps/shared/scripts/install_codegen_venv.sh"
INSTALL_VENV_CMD="$INSTALL_VENV_SCRIPT --walberla-root \"$SRC_DIR\" --project-root \"$PROJECT_ROOT\" --venv \"$VENV\""
...
[[ -f "$INSTALL_VENV_SCRIPT" ]] || { echo "ERROR: Missing venv installer script: $INSTALL_VENV_SCRIPT"; exit 1; }

# Optional srun wrapper. Guard with FLUIDSIM_BUILD_INNER to avoid recursion.
if [[ "${BUILD_USE_SRUN}" == "1" && "${FLUIDSIM_BUILD_INNER:-0}" != "1" ]]; then
    ...
    echo "Using srun wrapper (BUILD_USE_SRUN=1)"
    printf 'Resolved srun command: srun'
    printf ' %q' "${SRUN_ARGS[@]}"
    printf ' %q' /usr/bin/env \
    ...
    exec srun "${SRUN_ARGS[@]}" /usr/bin/env \
        FLUIDSIM_BUILD_INNER=1 \
        BUILD_USE_SRUN="$BUILD_USE_SRUN" \
        RECONFIGURE="$RECONFIGURE" \
        BUILD_CPUS_PER_TASK="$BUILD_CPUS_PER_TASK" \
...
if command -v module >/dev/null 2>&1 && [[ -n "$TOOLCHAIN_MODULE$PYTHON_MODULE$MPI_MODULE$CMAKE_MODULE" ]]; then
    module purge
...
PY
then
    echo "ERROR: Required codegen dependencies are missing or incomplete in $VENV" >&2
    echo "Run exactly:" >&2
    echo "  $INSTALL_VENV_CMD" >&2
    exit 1
fi
...
CMAKE_ARGS=(
    ...
    -DPystencilsSfg_PYTHON_INTERPRETER="$VENV/bin/python"
...
printf 'Resolved configure command: cmake -S %q -B %q' "$SRC_DIR" "$BUILD_DIR"
printf ' %q' "${CMAKE_ARGS[@]}"
printf '\n'
...
printf 'Resolved build command: cmake --build %q --target %q --parallel %q\n' \
    "$BUILD_DIR" "$TARGET" "$BUILD_CPUS_PER_TASK"
