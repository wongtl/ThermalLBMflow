...
"""FluidSim GPU code-generation entry point."""
...
with SourceFileGenerator(keep_unknown_argv=True) as sfg:
    ...
    build_cfg.target = ps.Target.CUDA
    ...
    lbm_bulk = LbmBulk(sfg, "LBM", lbm_config, lbm_optimisation=lbm_optimisation)

    # LBM sweeps (GPU generates serial variants only).
